...
abstract class AbstractMessageController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('musharemodule_message_' . $templateParameters['routeArea'] . 'view');
    }
    /**
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'musharemodule_message_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('mu_share_module.form.handler.message');
    ...
    protected function deleteInternal(Request $request, MessageEntity $message, $isAdmin = false)
    {
        ...
        $logArgs = ['app' => 'MUShareModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'message', 'id' => $message->getKey()];
        ...
        $redirectRoute = 'musharemodule_message_' . ($isAdmin ? 'admin' : '') . 'view';
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'MUShareModule', 'user' => $userName, 'action' => $action, 'entity' => 'message', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'MUShareModule', 'user' => $userName, 'entity' => 'message', 'id' => $itemId]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'MUShareModule', 'user' => $userName, 'action' => $action, 'entity' => 'message', 'id' => $itemId]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('musharemodule_message_display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('musharemodule_message_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
