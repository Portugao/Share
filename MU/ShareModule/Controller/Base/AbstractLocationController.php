...
abstract class AbstractLocationController extends AbstractController
{
    ...
    protected function indexInternal(Request $request, $isAdmin = false)
    {
        ...
        return $this->redirectToRoute('musharemodule_location_' . $templateParameters['routeArea'] . 'view');
    }
    /**
    ...
    protected function viewInternal(Request $request, $sort, $sortdir, $pos, $num, $isAdmin = false)
    {
        ...
        $sortableColumns = new SortableColumns($this->get('router'), 'musharemodule_location_' . ($isAdmin ? 'admin' : '') . 'view', 'sort', 'sortdir');
    ...
    protected function editInternal(Request $request, $isAdmin = false)
    {
        ...
        $formHandler = $this->get('mu_share_module.form.handler.location');
    ...
    protected function deleteInternal(Request $request, LocationEntity $location, $isAdmin = false)
    {
        ...
        $logArgs = ['app' => 'MUShareModule', 'user' => $this->get('zikula_users_module.current_user')->get('uname'), 'entity' => 'location', 'id' => $location->getKey()];
        ...
        $formHook = $hookHelper->callFormDisplayHooks($form, $location, FormAwareCategory::TYPE_DELETE);
        
        if ($form->handleRequest($request)->isValid()) {
            if ($form->get('delete')->isClicked()) {
                // Let any ui hooks perform additional validation actions
                $validationErrors = $hookHelper->callValidationHooks($location, UiHooksCategory::TYPE_VALIDATE_DELETE);
                if (count($validationErrors) > 0) {
                    ...
                    $hookHelper->callFormProcessHooks($form, $location, FormAwareCategory::TYPE_PROCESS_DELETE);
                    
                    // Let any ui hooks know that we have deleted the location
                    $hookHelper->callProcessHooks($location, UiHooksCategory::TYPE_PROCESS_DELETE);
    ...
    protected function handleSelectedEntriesActionInternal(Request $request, $isAdmin = false)
    {
        ...
        foreach ($items as $itemId) {
            ...
            try {
                ...
                $logger->error('{app}: User {user} tried to execute the {action} workflow action for the {entity} with id {id}, but failed. Error details: {errorMessage}.', ['app' => 'MUShareModule', 'user' => $userName, 'action' => $action, 'entity' => 'location', 'id' => $itemId, 'errorMessage' => $exception->getMessage()]);
            }
            ...
            if ($action == 'delete') {
                $this->addFlash('status', $this->__('Done! Item deleted.'));
                $logger->notice('{app}: User {user} deleted the {entity} with id {id}.', ['app' => 'MUShareModule', 'user' => $userName, 'entity' => 'location', 'id' => $itemId]);
            } else {
                $this->addFlash('status', $this->__('Done! Item updated.'));
                $logger->notice('{app}: User {user} executed the {action} workflow action for the {entity} with id {id}.', ['app' => 'MUShareModule', 'user' => $userName, 'action' => $action, 'entity' => 'location', 'id' => $itemId]);
            }
            ...
            if ($action != 'delete') {
                ...
                $url = new RouteUrl('musharemodule_location_display', $urlArgs);
            }
            $hookHelper->callProcessHooks($entity, $hookType, $url);
        }
        
        return $this->redirectToRoute('musharemodule_location_' . ($isAdmin ? 'admin' : '') . 'index');
    }
}
