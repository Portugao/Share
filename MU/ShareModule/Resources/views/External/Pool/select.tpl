...
{assign var='baseID' value='pool'}
<div id="itemSelectorInfo" class="hidden" data-base-id="{$baseID}" data-selected-id="{$selectedId|default:0}"></div>
<div class="row">
    <div class="col-sm-8">
        <div class="form-group">
            <label for="{$baseID}Id" class="col-sm-3 control-label">{gt text='Pool'}:</label>
            <div class="col-sm-9">
                <select id="{$baseID}Id" name="id" class="form-control">
                    {foreach item='pool' from=$items}
                        <option value="{$pool->getKey()}"{if $selectedId eq $pool->getKey()} selected="selected"{/if}>{$pool->getCollectionOfPool()}</option>
                    {foreachelse}
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <select id="{$baseID}Sort" name="sort" class="form-control">
                    <option value="collectionOfPool"{if $sort eq 'collectionOfPool'} selected="selected"{/if}>{gt text='Collection of pool'}</option>
    ...
    <div class="col-sm-4">
        <div id="{$baseID}Preview" style="border: 1px dotted #a3a3a3; padding: .2em .5em">
            <p><strong>{gt text='Pool information'}</strong></p>
