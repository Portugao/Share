        ...
        {if $properties ne null && is_array($properties)}
            ...
            {foreach key='propertyName' item='propertyId' from=$properties}
                <div class="form-group">
                    {assign var='hasMultiSelection' value=$categoryHelper->hasMultipleSelection('offer', $propertyName)}
                    ...
                    <div class="col-sm-9">
                        {selector_category name="`$baseID`_`$categorySelectorName``$propertyName`" field='id' selectedValue=$catIds.$propertyName|default:null categoryRegistryModule='MUShareModule' categoryRegistryTable="`$objectType`Entity" categoryRegistryProperty=$propertyName defaultText=$lblDefault editLink=false multipleSize=$categorySelectorSize cssClass='form-control'}
                    </div>
                </div>
            {/foreach}
            {/nocache}
            ...
            <div class="col-sm-9">
                <select id="{$baseID}Id" name="id" class="form-control">
                    {foreach item='offer' from=$items}
                        <option value="{$offer->getKey()}"{if $selectedId eq $offer->getKey()} selected="selected"{/if}>{$offer->getProduct()}</option>
                    {foreachelse}
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <select id="{$baseID}Sort" name="sort" class="form-control">
                    <option value="product"{if $sort eq 'product'} selected="selected"{/if}>{gt text='Product'}</option>
                    <option value="description"{if $sort eq 'description'} selected="selected"{/if}>{gt text='Description'}</option>
                    <option value="isOpen"{if $sort eq 'isOpen'} selected="selected"{/if}>{gt text='Is open'}</option>
                    <option value="abo"{if $sort eq 'abo'} selected="selected"{/if}>{gt text='Abo'}</option>
                    <option value="period"{if $sort eq 'period'} selected="selected"{/if}>{gt text='Period'}</option>
                    <option value="totalYear"{if $sort eq 'totalYear'} selected="selected"{/if}>{gt text='Total year'}</option>
                    <option value="splitCosts"{if $sort eq 'splitCosts'} selected="selected"{/if}>{gt text='Split costs'}</option>
                    <option value="back"{if $sort eq 'back'} selected="selected"{/if}>{gt text='Back'}</option>
                    <option value="atMe"{if $sort eq 'atMe'} selected="selected"{/if}>{gt text='At me'}</option>
                    <option value="zipCode"{if $sort eq 'zipCode'} selected="selected"{/if}>{gt text='Zip code'}</option>
                    <option value="meetingPlace"{if $sort eq 'meetingPlace'} selected="selected"{/if}>{gt text='Meeting place'}</option>
