            ...
            <div class="col-sm-9">
                <select id="{$baseID}Id" name="id" class="form-control">
                    {foreach item='message' from=$items}
                        <option value="{$message->getKey()}"{if $selectedId eq $message->getKey()} selected="selected"{/if}>{$message->getSubject()}</option>
                    {foreachelse}
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <select id="{$baseID}Sort" name="sort" class="form-control">
                    ...
                    <option value="recipient"{if $sort eq 'recipient'} selected="selected"{/if}>{gt text='Recipient'}</option>
