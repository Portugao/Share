...
namespace MU\ShareModule\Form\Handler\Pool\Base;

use MU\ShareModule\Form\Handler\Common\EditHandler;
use MU\ShareModule\Form\Type\PoolType;
 ...
 * It aims on the pool object type.
 */
abstract class AbstractEditHandler extends EditHandler
{
    ...
    {
        $this->objectType = 'pool';
        $this->objectTypeCapital = 'Pool';
        $this->objectTypeLower = 'pool';
        ...
        if ($this->templateParameters['mode'] == 'create') {
            if (!$this->modelHelper->canBeCreated($this->objectType)) {
                $this->request->getSession()->getFlashBag()->add('error', $this->__('Sorry, but you can not create the pool yet as other items are required which must be created before!'));
    ...
    protected function createForm()
    {
        ...
        return $this->formFactory->create(PoolType::class, $this->entityRef, $options);
    }
    ...
    protected function getRedirectCodes()
    {
        ...
        // user index page of pool area
        $codes[] = 'userIndex';
        // admin index page of pool area
        $codes[] = 'adminIndex';
        // user list of pools
        $codes[] = 'userView';
        // admin list of pools
        ...
        $codes[] = 'adminOwnView';
        // user detail page of treated pool
        $codes[] = 'userDisplay';
        // admin detail page of treated pool
        $codes[] = 'adminDisplay';
    ...
    protected function getDefaultMessage(array $args = [], $success = false)
    {
        ...
        switch ($args['commandName']) {
            case 'submit':
                if ($this->templateParameters['mode'] == 'create') {
                    $message = $this->__('Done! Pool created.');
                } else {
                    $message = $this->__('Done! Pool updated.');
                }
                break;
            case 'delete':
                $message = $this->__('Done! Pool deleted.');
                break;
            default:
                $message = $this->__('Done! Pool updated.');
                break;
    ...
    public function applyAction(array $args = [])
    {
        ...
        try {
            ...
            $logArgs = ['app' => 'MUShareModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => 'pool', 'id' => $entity->getKey(), 'errorMessage' => $exception->getMessage()];
