...
abstract class AbstractThirdPartyListener implements EventSubscriberInterface
{
    ...
    public function contentGetTypes(\Zikula_Event $event)
    {
        ...
        $types->add('MUShareModule_ContentType_Item');
        
        // plugin for showing a list of multiple items
        $types->add('MUShareModule_ContentType_ItemList');
    }
     ...
     * MUShareModule will use this to add a javascript helper to add custom items.
     *
     * You can access general data available in the event.
     *
     * The event name:
    ...
    public function getEditorHelpers(EditorHelperEvent $event)
    {
        ...
        $targetDir = 'web/modules/mushare';
        $finder = new Finder();
        if (!$this->filesystem->exists($targetDir)) {
            $this->filesystem->mkdir($targetDir, 0777);
            if (is_dir($originDir = 'modules/MU/ShareModule/Resources/public')) {
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
            if (is_dir($originDir = 'modules/MU/ShareModule/Resources/scribite')) {
                $targetDir .= '/scribite';
                $this->filesystem->mkdir($targetDir, 0777);
                $this->filesystem->mirror($originDir, $targetDir, Finder::create()->in($originDir));
            }
        ...
        $event->getHelperCollection()->add(
            [
                ...
                'type' => 'javascript',
                'path' => $this->request->getBasePath() . '/web/modules/mushare/js/MUShareModule.Finder.js'
            ]
        );
    }
    ...
    public function getCKEditorPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'musharemodule',
            'path' => $this->request->getBasePath() . '/web/modules/mushare/scribite/CKEditor/musharemodule/',
            'file' => 'plugin.js',
            'img' => 'ed_musharemodule.gif'
        ]);
    }
    ...
    public function getQuillPlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'musharemodule',
            'path' => $this->request->getBasePath() . '/web/modules/mushare/scribite/Quill/musharemodule/plugin.js'
        ]);
    }
    ...
    public function getSummernotePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'musharemodule',
            'path' => $this->request->getBasePath() . '/web/modules/mushare/scribite/Summernote/musharemodule/plugin.js'
        ]);
    }
    ...
    public function getTinyMcePlugins(GenericEvent $event)
    {
        $event->getSubject()->add([
            'name' => 'musharemodule',
            'path' => $this->request->getBasePath() . '/web/modules/mushare/scribite/TinyMce/musharemodule/plugin.js'
        ]);
    }
}
